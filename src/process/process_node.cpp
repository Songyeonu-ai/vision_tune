    ...
    : Node("process_node")
{
  ...
  yolo_image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(
      yolo_image_topic, 1);
...
void ProcessNode::declare_parameters() // ui_config.yaml
{
  ...
  declare_parameter<std::string>("topic.yolo_image_topic", "/vision/yolo_image");
...
void ProcessNode::get_parameters()
{
  ...
  get_parameter("topic.yolo_image_topic", yolo_image_topic);
...
void ProcessNode::process_tick()
{
  ...
  // ========================= publish =========================
  auto yolo_img =
      cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", yolo_mat).toImageMsg();
...
std::vector<vision_tune::utils::detection_result> ProcessNode::detect_obstacle(const cv::Mat &raw_mat)
{
  ...
  for (size_t i = 0; i < n_det; i++)
  {
    ...
    if (static_cast<int>(d[5]) != 0)
      continue;

    float conf = d[4];
    if (conf < CONFIDENCE_THRESHOLDS.at(0))
      continue;
